...
/*  (behind the hidden mount prefix) exchanges the DECK password for   */
/*  a 12h HMAC bearer token scoped to deck routes only - it cannot     */
/*  touch content-admin endpoints, and an admin token cannot drive     */
/*  relays. Server-side rate limit: 5 attempts / 15 min per vault.     */
/*  Storage key "rk-deck-token" is kept separate from the admin user's */
/*  "rk-admin-token". The token lives in sessionStorage only, so it    */
/*  never survives on disk and dies when the browser/app closes.       */
/*                                                                     */
...
export function useDeckAuth(): DeckAuth {
  ...
  const login = useCallback(async (password: string) => {
    ...
    sessionStorage.setItem(DECK_TOKEN_KEY, token);
    setToken(token);
    setLockReason(null);
  }, []);

  const reauth = useCallback((fresh: string) => {
    sessionStorage.setItem(DECK_TOKEN_KEY, fresh);
    setToken(fresh);
    setLockReason(null);
  }, []);
